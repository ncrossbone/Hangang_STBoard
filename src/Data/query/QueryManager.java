...
public class QueryManager {
	...
	public String getQueryString(int pageNum, HttpServletRequest request) {
		...
		case 1:
			...
			queryString
					.append("   ST_AsGeoJSON(ST_Transform(geometry(geog),900913)) as geoJson,  ");
		...
		case 2:
			...
			queryString
					.append(" 'hacheon' || TO_CHAR(A.GID, 'FM9999') AS id, ");
			queryString
					.append(" ST_AsGeoJSON(ST_Transform(geometry(geog),900913)) as geoJson, ");
			...
			queryString.append("  ");
			queryString
					.append(" SELECT     *   FROM     PUBLIC.HACHEONSU  WHERE      ");
			queryString
					.append(" \"측정소명\" in ('영월2', '충주댐', '달천4', '섬강4-1',      ");
			queryString
					.append(" '강천', '강상', '경안천5', '소양강2', '의암댐', '삼봉리',      ");
		...
		case 4:
			...
			queryString
					.append(" 	array_to_json(array_agg(row_to_json(a))) AS features from ");
			queryString.append(" 	( ");
			queryString.append(" 		SELECT    ");
			queryString.append(" 		'Feature' as type, ");
			queryString
					.append(" 		'bo.' || to_char(HACHEONSU.gid, 'FM9999') as id,     ");
			queryString
					.append(" 		ST_AsGeoJSON(ST_Transform(geometry(geog),900913))::json as geometry,   ");
			queryString.append(" 		'geog' as geometry_name,   ");
			queryString.append(" 		( ");
			...
			queryString
					.append(" 			WHERE \"측정소명\" in ('영월2', '충주댐', '달천4', '섬강4-1',       '강천', '강상', '경안천5', '소양강2', '의암댐', '삼봉리',       '팔당댐', '노량진', '가양', '임진강4', '안성천3') ");
			queryString.append(" 		) AS HACHEONSU,     ");
			queryString.append(" 		public.HACHEON_POINT_DATA ");
			queryString
					.append(" 		WHERE     HACHEONSU.gid =  HACHEON_POINT_DATA.GID ");
		...
		case 5:
			...
			queryString.append(" 			ON sujil.\"측정소코드\" = DATA.\"측정소코드\" "
					+ spotGubun);
			queryString.append(" 	) as a ");
			queryString.append(" ) as t ");
		...
		case 6:
			...
			queryString.append(" 			(  ");
			queryString
					.append(" 				SELECT row_to_json(PROPERTY_TMP) from (  ");
			queryString.append(" 					SELECT " + value + " from   ");
		...
		case 9:
			...
			queryString.append("     select 'FeatureCollection' AS type, ");
			queryString
					.append("     array_to_json(array_agg(row_to_json(a))) AS features ");
			queryString.append("     from ( ");
			...
			queryString.append("         ( ");
			queryString
					.append("             SELECT row_to_json(PROPERTY_TMP) ");
			queryString.append("             from ( ");
			queryString
					.append("                 SELECT \"저수량(㎥/s)\" as TODAY_DATA, ");
			...
			queryString
					.append("                 where \"측정코드\" = soomun.\"관측소코드\"  ");
			queryString.append("             ) as PROPERTY_TMP  ");
			queryString.append("         ) AS PROPERTIES ");
			queryString
					.append("         from soomun INNER JOIN \"수문정보_데이터\" AS DATA ON soomun.\"관측소코드\" = DATA.\"측정코드\"  ");
			queryString.append("     ) as a  ");
			queryString.append(" ) as t  ");
		...
		case 18:
			...
			queryString
					.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT FROM (SELECT \"측정지점명\" as pointName, "
							+ colName + " as data FROM \"수질측정망_데이터\") AS A ");
		...
		case 19:
			...
			queryString
					.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT FROM (SELECT \"측정지점명\" as pointName, "
							+ colName + " as data FROM \"자동측정망_데이터\") AS A ");
		...
		case 20:
			...
			queryString
					.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT FROM (SELECT \"측정소명\" as pointName, "
							+ colName + " as data FROM \"조류측정망_데이터\") AS A ");
		...
		case 21:
			...
			queryString
					.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT FROM (SELECT \"측정소명\" as pointName, "
							+ colName + " as data FROM \"ip_usn_데이터\") AS A ");
		...
		case 22:
			...
			queryString
					.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT FROM (SELECT \"측정지점명\" as pointName, "
							+ colName + " as data FROM \"수문정보_데이터\") AS A ");
		...
		case 23:
			...
			queryString.append("                            WHEN \"SOO\" IS NULL THEN '' ") ;
			queryString.append("                            ELSE \"SOO\" ") ;
			queryString.append("                          END AS \"SOO\", ") ;
			queryString.append("                          CASE ") ;
			queryString.append("                            WHEN \"col_dd_dt\" IS NULL THEN '0' ") ;
			queryString.append("                            WHEN trim(\"col_dd_dt\") = '' THEN '0' ") ;
			queryString.append("                            ELSE \"col_dd_dt\" ") ;
			queryString.append("                          END AS \"COL_DD_DT\", ") ;
			queryString.append("                          CASE ") ;
			queryString.append("                            WHEN \"col_mm_dt\" IS NULL THEN '0' ") ;
			queryString.append("                            WHEN trim(\"col_mm_dt\") = '' THEN '0' ") ;
			queryString.append("                            ELSE \"col_mm_dt\" ") ;
			queryString.append("                          END AS \"COL_MM_DT\", ") ;
			queryString.append("                          CASE ") ;
			queryString.append("                            WHEN \"col_yr_dt\" IS NULL THEN '0' ") ;
			queryString.append("                            WHEN trim(\"col_yr_dt\") = '' THEN '0' ") ;
			...
			queryString.append("                      AND \"MSM_LT_CD\" = 'HM0001' LIMIT 1 ) AS COL_DD_DT , ") ;
			queryString.append("                  ( ") ;
			...
			queryString.append("                      AND \"MSM_LT_CD\" = 'HM0001' LIMIT 1 ) AS COL_MM_DT , ") ;
			queryString.append("                  ( ") ;
			...
			queryString.append("                      AND \"MSM_LT_CD\" = 'HM0001' LIMIT 1 ) AS COL_YR_DT, ") ;
			queryString.append("                  ( ") ;
			queryString.append("                    SELECT \"환경기준\" ") ;
			queryString.append("                    FROM public.soo ") ;
			queryString.append("                    WHERE \"MMP_INFO\".\"MMP_CD\" = public.soo.\"측정소코드\" ) AS \"SOO\" ") ;
			queryString.append("                FROM \"MMP_INFO\" ") ;
			queryString.append("                WHERE \"PT_DIV_CD\" = '"+measure+"' ") ;
			queryString.append("                  and \"MW_CD\"='"+code+"' ) AS D ) AS C ) AS A ) AS T ") ;


			

			System.out.println("23 Query :" + queryString.toString());
			break;

		case 30: // top 맵 extent
			// System.out.println("30Query");
			queryString
					.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT FROM (SELECT mw_name, mw_code FROM junghangang  where ws_name='한강') AS A");
		...
		case 40: // 마커랑 차트 검색 쿼리
			...
			// String prevMonth = dateFormat.format(calendar.add(Calendar.MONTH,
			// -1));
			...
			queryString.append(" FROM ");
			queryString
					.append("   (SELECT 'FeatureCollection'                AS type, ");
			queryString
					.append("     array_to_json(array_agg(row_to_json(a))) AS features ");
			queryString.append("   FROM ");
			queryString
					.append("     (SELECT 'Feature'                                           AS TYPE, ");
			queryString
					.append("       ST_AsGeoJSON(ST_Transform(geometry(\"GEOG\"),900913))::json AS geometry, ");
			queryString
					.append("       'geog'                                                    AS GEOMETRY_NAME, ");
			queryString
					.append("       ROW_TO_JSON(C)                                            AS PROPERTIES ");
			queryString.append("     FROM ");
			queryString.append("       (SELECT \"MMP_CD\" , ");
			queryString.append("         \"PT_DIV_CD\" , ");
			queryString.append("         \"MW_CD\" , ");
			...
			queryString.append("           FROM \"COL_DD_DT\" ");
			queryString
					.append("           WHERE \"MMP_INFO\".\"MMP_CD\" = \"COL_DD_DT\".\"MMP_CD\" ");
			queryString.append("           AND \"MSM_LT_CD\"           = '"
					+ measure + "' LIMIT 1 ");
			queryString.append("           ) AS COL_DD_DT , ");
			queryString.append("           (SELECT \"COL_MM_DT\" ");
			queryString.append("           FROM \"COL_MM_DT\" ");
			queryString
					.append("           WHERE \"MMP_INFO\".\"MMP_CD\" = \"COL_MM_DT\".\"MMP_CD\" ");
			queryString.append("           AND \"MSM_LT_CD\"           = '"
					+ measure + "' LIMIT 1 ");
			...
			queryString.append("           FROM \"COL_YR_DT\" ");
			queryString
					.append("           WHERE \"MMP_INFO\".\"MMP_CD\" = \"COL_YR_DT\".\"MMP_CD\" ");
			queryString.append("           AND \"MSM_LT_CD\"           = '"
					+ measure + "' LIMIT 1 ");
			...
			queryString.append("           FROM public.soo ");
			queryString
					.append("           WHERE \"MMP_INFO\".\"MMP_CD\" = public.soo.\"측정소코드\" ");
		...
		case 42: // FooterPanel
			...
			// 14.07.22 박순형 검색 쿼리 수정
			queryString
					.append(" SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT ");
			queryString.append(" FROM ");
		...
		case 43: 
			String mmpcd = request.getParameter("mmpcd");
			measure = request.getParameter("measure");
			...
			queryString.append("    SELECT * ") ;
			queryString.append("    FROM \"COL_DD_DT\" ") ;
			queryString.append("    WHERE \"MMP_CD\" ='"+mmpcd+"' ") ;
			queryString.append("    ORDER BY \"PT_DIV_CD\", \"MSM_LT_CD\") AS A ") ;
			
			/*queryString.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT ") ;
			queryString.append("FROM ( ") ;
			queryString.append("    SELECT * ") ;
			queryString.append("    FROM \"COL_DD_DT\" ") ;
			queryString.append("    WHERE \"MMP_CD\" = '"+mmpcd+"' ) AS A ") ;*/
			System.out.println("43 Query : " + queryString.toString());
			break;
		...
		case 44: 
			...
			queryString.append("SELECT array_to_json(array_agg(row_to_json(A))) AS RESULT ") ;
			queryString.append("from ( ") ;
			queryString.append("    select Min(\"DATE\") , ") ;
			queryString.append("      Max(\"DATE\") ") ;
			queryString.append("    from \"COL_DD_DT\" ") ;
			queryString.append("    where \"PT_DIV_CD\" = '"+measure+"')a ") ;

			System.out.println("44 Query : " + queryString.toString());
			break;
		...
		case 51: // Top
			...
			queryString.append(" SELECT  ");
			queryString
					.append(" array_to_json(array_agg(row_to_json(result))) AS RESULT  ");
			queryString.append(" FROM ( ");
			queryString.append(" SELECT  ");
			queryString
					.append(" A.\"MMP_CD\" AS \"mmpCd\", A.\"MW_CD\" AS \"mwCd\", A.\"MMP_NM\" AS \"mmpNm\", ");
			...
			queryString.append("  FROM ( ");
			queryString
					.append(" 	SELECT * FROM \"MMP_INFO\" WHERE \"PT_DIV_CD\"='002' ");
			queryString.append(" ) AS A ");
			queryString.append(" INNER JOIN ( ");
			queryString.append(" 	SELECT * FROM \"COL_DD_DT\" ");
			queryString.append(" ) AS B ");
			queryString
					.append(" ON A.\"MMP_CD\" = B.\"MMP_CD\" AND A.\"PT_DIV_CD\" = B.\"PT_DIV_CD\" ");
			queryString.append(" INNER JOIN ( ");
			queryString.append(" 	SELECT * FROM \"MW_TRG_STRD\" ");
			queryString.append(" ) AS C ");
			queryString
					.append(" ON A.\"MW_CD\" = C.\"MW_CD\" AND B.\"MSM_LT_CD\" = C.\"MSM_LT_CD\" ");
		...
		case 52:
			...
			// queryString.append(" 		\"AT_ORD\" ");
			// queryString.append(" 	FROM \"MMP_INFO\"    ");
			// queryString.append(" 	WHERE \"PT_DIV_CD\"='" +
			// request.getParameter("spotValue") +
			// "'  AND \"AT_ORD\" IS NOT NULL ");
	...
	/*		queryString
					.append(" SELECT array_to_json(array_agg(row_to_json(result))) AS RESULT ");
			queryString.append(" FROM ");
			...
			queryString.append("     ( ");
			queryString
					.append("     SELECT ST_X(ST_AsText(ST_Transform(geometry(\"GEOG\"),900913))) AS X, ");
			queryString
					.append("       ST_Y(ST_AsText(ST_Transform(geometry(\"GEOG\"),900913)))       AS Y, ");
			queryString.append("       \"MMP_NM\", ");
			queryString.append("       \"AT_ORD\" ");
			queryString.append("     FROM \"MMP_INFO\" ");
			queryString.append(" 	WHERE \"PT_DIV_CD\"='"
			...
			queryString.append(" 	WHERE \"PT_DIV_CD\"='"
					...
					+ "'  AND \"AT_ORD\" IS NOT NULL ");
			queryString.append("     ) AS A ");
			queryString.append("    ORDER BY \"AT_ORD\")AS result ") ;
		...
		case 53: // new Top
			...
			queryString
					.append(" ROW_NUMBER() OVER( PARTITION BY A.\"MMP_CD\" , B.\"MSM_LT_CD\" ORDER BY B.\"DATE\" DESC ) AS CNT   ");
			queryString.append(" FROM ( ");
			queryString
					.append(" SELECT * FROM \"MMP_INFO\" WHERE \"PT_DIV_CD\" = '"
							+ spotGubun + "') AS A ");
			queryString.append(" INNER JOIN ( ");
		...
		case 70:
			...
			queryString.append(" INSERT INTO \"test\" (cd, nm, val) VALUES ('"
					+ ww1 + "', '" + ww2 + "', '" + ww3 + "') ");
		...
		case 71: // 년 데이터
			...
			queryString
					.append(" INSERT INTO \"COL_YR_DT_TEMP\" (\"MMP_CD\", \"MSM_LT_CD\", \"DATE\", \"PT_DIV_CD\", \"COL_YR_DT\") VALUES ( '"
							+ w1
							+ "', '"
							+ w2
							+ "', '"
		...
		case 72: // 월 데이터
			...
			System.out.println(w5);
			queryString
					.append(" INSERT INTO \"COL_MM_DT_TEMP\" (\"MMP_CD\", \"MSM_LT_CD\", \"DATE\", \"PT_DIV_CD\", \"COL_MM_DT\") VALUES ( '"
							+ w1
							+ "', '"
							+ w2
							+ "', '"
		...
		case 73: // 일 데이터
			...
			System.out.println(w5);
			queryString
					.append(" INSERT INTO \"COL_DD_DT_TEMP2\" (\"MMP_CD\", \"MSM_LT_CD\", \"DATE\", \"PT_DIV_CD\", \"COL_DD_DT\") VALUES ( '"
							+ w1
							+ "', '"
							+ w2
							+ "', '"
